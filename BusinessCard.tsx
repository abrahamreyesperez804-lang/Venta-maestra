...
export const BusinessCard: React.FC<BusinessCardProps> = ({ business, onDeleteRequest }) => {
  const { name, category, location, description, phone, website } = business;
    ...
    <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-900/40 hover:-translate-y-1 flex flex-col">
      <div className="p-5 flex-grow">
        ...
        {(phone || website) && (
            ...
            {phone && (
                <a href={`tel:${phone}`} className="flex items-center text-sm text-gray-400 hover:text-indigo-400 transition-colors">
                <PhoneIcon className="w-4 h-4 mr-2 flex-shrink-0 text-gray-500" />
                <span>{phone}</span>
                </a>
            )}
            {website && (
                <a href={website} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm text-gray-400 hover:text-indigo-400 transition-colors truncate">
                <LinkIcon className="w-4 h-4 mr-2 flex-shrink-0 text-gray-500" />
